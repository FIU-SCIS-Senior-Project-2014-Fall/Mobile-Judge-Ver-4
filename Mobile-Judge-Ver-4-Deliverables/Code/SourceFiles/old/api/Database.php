...
class Database{
    ...
    public function connect(){
        ...
        if(!$this->con){
            $myconn = @mysql_connect($this->db_host,$this->db_user,$this->db_pass) or die(mysql_error());  // mysql_connect() with variables defined at the start of Database class
            if($myconn){
                $seldb = @mysql_select_db($this->db_name,$myconn) or die(mysql_error()); // Credentials have been pass through mysql_connect() now select the database
                if($seldb){
                    $this->con = true;
